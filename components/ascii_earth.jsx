...
export default function AsciiEarthOptimized() {
  ...
  useEffect(() => {
    ...
    // Earth parameters
    const radius = Math.min(width, height) * 0.45
    const cellSize = 8 // Larger cell size for better performance (was 6)
    ...
    // Create a simplified world map (90x180 array representing lat/lng) - reduced resolution
    // 1 = land, 0 = ocean
    if (!worldMapRef.current) {
      worldMapRef.current = new Array(90).fill().map(() => new Array(180).fill(0))
    }
    ...
    worldMapImage.crossOrigin = "anonymous" // Avoid CORS issues
    worldMapImage.src =
      "globe.jpg" // Using the provided image 
    worldMapImage.onload = () => {
      setLoadingProgress("Processing map image...")

      // Create a temporary canvas to process the image
      const tempCanvas = document.createElement("canvas")
      tempCanvas.width = 180 // Reduced resolution (was 360)
      tempCanvas.height = 90 // Reduced resolution (was 180)
      const tempCtx = tempCanvas.getContext("2d")

      // Draw the world map image to the temporary canvas, scaling it to match our worldMap dimensions
      tempCtx.drawImage(worldMapImage, 0, 0, 180, 90)

      // Get the image data
      const imageData = tempCtx.getImageData(0, 0, 180, 90)
      const data = imageData.data

      // Process the image data to create the worldMap array
      for (let y = 0; y < 90; y++) {
        for (let x = 0; x < 180; x++) {
          const idx = (y * 180 + x) * 4
          // Check if the pixel is land (green) vs water (white)
          ...
          // Improved land detection with stronger threshold
          if (g > r + 30 && g > b + 30 && g > 100) {
            worldMapRef.current[y][x] = 1 // Land
          }
        }
      }
    ...
    function defineSimplifiedContinents() {
      // Simplified continent definitions for fallback
      // North America
      for (let lat = 15; lat <= 37; lat++) {
        const width = Math.floor(12 + (lat - 15) * 0.25)
        const center = 47
        for (let lng = center - width; lng <= center + width; lng++) {
          if (lat >= 0 && lat < 90 && lng >= 0 && lng < 180) {
            worldMapRef.current[lat][lng] = 1
          }
        }
      }

      // South America
      for (let lat = 37; lat <= 55; lat++) {
        const width = Math.floor(7 - (lat - 37) * 0.25)
        const center = 60
        for (let lng = center - width; lng <= center + width; lng++) {
          if (lat >= 0 && lat < 90 && lng >= 0 && lng < 180) {
            worldMapRef.current[lat][lng] = 1
    ...
    function drawASCIIEarth() {
      ...
      if (isLoading) {
        ...
        ctx.textBaseline = "middle"
        ctx.fillText(loadingProgress, width / 2, height / 2)
        requestRef.current = requestAnimationFrame(drawASCIIEarth)
        return
      }
      ...
      for (let j = 0; j < gridHeight; j++) {
        for (let i = 0; i < gridWidth; i++) {
          ...
          if (dist < 1) {
            ...
            // Adjust for map array indices
            lat = Math.round((lat + 90) / 2) // Scale to our reduced resolution
            lng = Math.round((lng + 180) / 2) // Scale to our reduced resolution

            // Clamp to valid indices
            lat = Math.max(0, Math.min(89, lat))
            lng = Math.max(0, Math.min(179, lng))
            ...
            // Determine if this point is land or ocean using the world map
            const isOnLand = worldMapRef.current[lat][lng] === 1
      ...
      // Update rotation angle (slower rotation)
      frameCount++
      if (frameCount % 2 === 0) {
        // Update angle every other frame
        angle += 0.01 // Reduced from 0.02
      }
